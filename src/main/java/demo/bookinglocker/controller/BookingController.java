    ...
    public ResponseEntity<Object> addBooking(@RequestBody Booking booking, BindingResult result) {
        ...
        try {
            ...
            if (knownUser.isEmpty()|| knownUser.size()==0) {
                ...
                List<Booking> LockerBookByUser = bookingRepo.findListLockerByUserId(userId);
                if (LockerBookByUser.size()>=3) {
                    String Pesan = "User sudah booking 3 locker";
                    LOGGER.info("Block booking karena User : "+userId+" sudah booking 3 locker");
                    return responsehandler.generateResponse(Pesan,HttpStatus.OK, null);
                } else {
                ...
                if (availableLocker.isEmpty()|| availableLocker.size()==0) {
                    ...
                    return responsehandler.generateResponse(Pesan,HttpStatus.OK, null);
                        ...
                        if (availability == true) {
                            ...
                            lockerService.setPassword(lockerNumber);
                            ...
                            String Pesan="";
                            bookingService.addBooking(userId, lockerNumber, currentDate, currentDate, BigInteger.valueOf(10000), BigInteger.valueOf(0),1);
